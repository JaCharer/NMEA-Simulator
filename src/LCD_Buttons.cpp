...
void setupLCDButtons() {
    ...
    // Alokacja z weryfikacją - jeśli brakuje RAM, zobaczymy to w logach
    if (!sprSOG.createSprite(100, 30) || !sprCOG.createSprite(100, 30) || 
        !sprHeader.createSprite(130, 12) || !sprInfo.createSprite(130, 32) || 
        !sprStat.createSprite(130, 12)) {
        Serial.println("[LCD] BŁĄD KRYTYCZNY: Brak pamięci RAM dla Sprite!");
    }
    ...
    // Zwiększamy rozmiar stosu dla pewności przy operacjach na String/printf
    xTaskCreatePinnedToCore(guiTaskCode, "GUITask", 16384, NULL, 2, &GUITask, 0);
}
#endif
// =========================================================================
// EKRAN TFT i GUI
...
bool updateDisplay()
{
  ...
  else
  {
    ...
    return false; // Sygnalizujemy niepowodzenie
  }

  // 1. Dynamiczna aktualizacja nagłówka z adresem IP
  sprHeader.fillSprite(TFT_BLACK);
  sprHeader.setTextSize(1);
  sprHeader.setTextColor(TFT_YELLOW, TFT_BLACK);
  sprHeader.setCursor(0, 0);
  
  // Bardzo bezpieczne pobieranie IP - unika crashy przy braku init WiFi
  ...
  sprHeader.print(" IP: ");
  sprHeader.print(ipStr);
  sprHeader.pushSprite(2, 0);

  // 2. Aktualizacja danych jachtu
...
// === STANDARDOWA OBSŁUGA ZMIANY KURSU ===
    ...
    {
      // Jeśli updateDisplay się nie udał (brak Mutexu), 
      // nie aktualizujemy lastDispUpdate, aby spróbować ponownie w następnej pętli.
      if (updateDisplay()) {
