  ...
  webServer.on("/saveconfig", HTTP_POST, [](AsyncWebServerRequest *request) {}, nullptr, [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total)
      ...
      if (index + len == total) {
        ...
        if (wifiChanged) {
          ...
          request->send(200, "text/plain", "WiFi saved! ESP32 is restarting...");
          delay(2000);
          ESP.restart();
        } else {
          request->send(200, "text/plain", "Settings saved (no network changes)");
        }
      } });
  ...
  webServer.on("/settings", HTTP_GET, [](AsyncWebServerRequest *request)
               {
                 if (xSemaphoreTake(dataMutex, pdMS_TO_TICKS(50)))
                 {
                   ...
                   {
                     config.enableN2K = request->getParam("n2k")->value() == "1";
                     preferences.putBool("n2k", config.enableN2K);
                   }
                   xSemaphoreGive(dataMutex);
                   request->send(200);
                 }
