...
void setup()
{
  ...
  config.enableN2K = preferences.getBool("n2k", true);     // Włącz NMEA 2000
...
void loop()
{
  ...
  #ifdef SIMMULATE_NMEA2000
  if(config.enableN2K) runN2KUpdate(); // Obsługa stosu N2K (address claiming itp.)
  #endif
  ...
  if (millis() - lastTick >= 100)
  {
    ...
    if (xSemaphoreTake(dataMutex, pdMS_TO_TICKS(50)))
    {
     ...
     #ifdef SIMMULATE_NMEA2000
      if(config.enableN2K) sendN2KNavigation(snap);
    #endif   
    ...
    case 4:
  #ifdef SIMMULATE_NMEA2000
      if(config.enableN2K) sendN2KWind(snap);
  #endif
      break;
    ...
    case 7:
  #ifdef SIMMULATE_NMEA2000
      if(config.enableN2K) sendN2KDepth(snap);
  #endif
      break;
    ...
    case 9:
  #ifdef SIMMULATE_NMEA2000
      if(config.enableN2K) sendN2KEngine(snap);
  #endif
      break;
