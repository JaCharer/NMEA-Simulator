...
{ // Funkcja symulująca inteligencję skippera
  ...
  const double start_lat = 55.5000;
  const double start_lon = 18.0000;

  // Definicja wierzchołków kwadratu
  double wp_lat[4] = {start_lat + side_deg, start_lat + side_deg, start_lat, start_lat};
  double wp_lon[4] = {start_lon, start_lon + side_deg, start_lon + side_deg, start_lon};

  // 1. Obliczanie namiaru (bearing) do aktualnego punktu drogi
  double dy = wp_lat[snap.currentWaypoint] - snap.lat;
  double dx = (wp_lon[snap.currentWaypoint] - snap.lon) * cosf(snap.lat * PI / 180.0);
