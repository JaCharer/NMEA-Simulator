...
void updateYachtPhysics()
{
  ...
  double safe_lat = l_lat;
  double safe_lon = l_lon;
  double safe_sog = l_sog;
  double safe_cog = l_cog;
  ...
  // 1. Zmiana głębokości jako "random walk" (płynne opadanie/wznoszenie dna)
if (random(0, 5) == 0) // Co ok. 0.5 sekundy dno lekko się zmienia
{
  // Losujemy małą zmianę: od -0.3m do +0.3m
  float depth_change = (random(-30, 31) / 100.0f); 
  l_tgt_depth += depth_change;
  
  // Trzymamy głębokość w bezpiecznych granicach (np. od 2m do 150m)
  l_tgt_depth = constrain(l_tgt_depth, 2.0f, 150.0f);
}

// 2. Bardzo silny filtr dolnoprzepustowy (wygładzanie)
// Współczynnik 0.01 sprawi, że przejścia będą super płynne, bez skoków
l_depth += (l_tgt_depth - l_depth) * 0.01f;
